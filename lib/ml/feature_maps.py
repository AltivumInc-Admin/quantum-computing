...
def iqp_encoding(features: np.ndarray, reps: int = 2) -> Circuit:
    ...
    This is the workspace's canonical IQP convention, and the lesson explorer
    (``web/src/components/quantum/encoding.ts``) implements the SAME map: the
    two are pinned to each other through
    ``web/src/components/quantum/__fixtures__/iqp-states.json`` (generated by
    ``scripts/gen_iqp_fixture.py`` and asserted from both sides), so the kernel
    matrix a learner sees in the lesson widget is the one their notebook
    reproduces. It is a simplified variant of the Havlicek et al. ZZFeatureMap
    convention (arXiv:1804.11326), which uses ``phi_i = 2*x_i`` and
    ``phi_ij = 2*(pi - x_i)*(pi - x_j)``.

...
- [x] **Step 2: The flip** *(gate)*: in the HQ zone, UPSERT apex + `www` ALIAS/A records → the Amplify custom-domain target (from Task 7; CloudFront hosted-zone id `Z2FDTNDATAQYW2`). TTL is already low. The zone's MX/TXT/DKIM records are untouched. *(Executed 2026-08-30 — required first releasing the CloudFront aliases held by the old `quantumlearner-dev-redirect` distribution and recreating the FAILED association; see operator log.)*
- [x] **Step 3: Verify live:** `dig +short A quantumlearner.dev @8.8.8.8` resolves to the new distribution; `curl -sI https://quantumlearner.dev/` → 200 with the new build id; login + tutor spot-check on the real domain; `curl -sI https://www.quantumlearner.dev/` → 200 or canonical redirect. *(2026-08-30: apex + www 200 verified; the login/tutor spot-check on the real domain rides the outstanding Task 11 steps.)*
- [x] **Step 4: The old name becomes a pointer** *(gate)*: ~~deploy `infra/redirect/quantum-altivum-ai.yaml`~~ *(superseded 2026-08-30: done by replacing the old Amplify app's custom rules with a single path-preserving 301 to `https://quantumlearner.dev` + a RELEASE job to flush edge caches — no new stack needed; the template ships dark. Verified: root, deep paths, and the default domain all 301.)*

...
- [ ] **[Tests/Ops]** ~reviewer (PARTIAL) — the Lambda handler/parseBody/corpus builder have zero tests; no Lambda log-retention is set; deploy has no corpus-freshness/model-id gate. DONE: a structured error log (`{tutorError:true,...}`) now lets a CloudWatch metric filter alarm on failures, and the corpus builder warns on truncation. STILL OPEN: Lambda handler unit tests, log-retention config, corpus-freshness/model-id deploy gate. Impact: Medium. Effort: Medium. (`lambda/tutor/`, `scripts/build_tutor_corpus.mjs`) — added 2026-06-17

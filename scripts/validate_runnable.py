...
# from one place rather than hand-edited in overrides.json.
...
def _qcsim_wheel_name() -> str:
    ...
    generated in any environment, and so the JupyterLite ``overrides.json`` pin
    is single-sourced rather than hand-edited.
    """
    version = "0.0.0"
    try:
        match = re.search(
...
def _generated_manifests() -> dict[Path, dict]:
    return {
        ...
        CONTENT_MANIFEST_PATH: build_content_manifest(),
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-manifest",
        action="store_true",
        help="(re)write the runnable + content manifests after a clean scan",
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help="exit 1 if either committed manifest is stale (CI drift gate; no write)",
    )
